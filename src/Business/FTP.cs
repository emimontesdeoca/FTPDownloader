...
namespace FtpDownloader.Business
{
    ...
    class FTP
    {
        ...
        public async Task<FtpWebRequest> CreateFtpWebRequest(string FTPDirectoryPath, string username, string password, bool keepAlive = false)
        {
            ...
            request = await Task.Run(() => (FtpWebRequest)FtpWebRequest.Create(new Uri(FTPDirectoryPath)));
            ...
            request.Credentials = await Task.Run(() => new NetworkCredential(username, password));
        ...
        {
            /// Creates FtpWebRequest.
            request = (FtpWebRequest)await CreateFtpWebRequest(FtpFolderPath, u, p, true);
            ...
            try
            {
                ...
                return await Task.Run(() => (FtpWebResponse)request.GetResponse());
        ...
        public async Task DownloadFile(string FtpFolderPath, string FtpUsername, string FtpPassword, string DownloadFolderPath, string UrlEncodedTorrent, string filename, string rootDownloadFolderPath)
        {
            ...
            request = (FtpWebRequest)await CreateFtpWebRequest(FtpFolderPath + UrlEncodedTorrent, u, p, true);
            ...
            try
            {
                using (Stream reader = request.GetResponse().GetResponseStream())
                {
                    ...
                    using (FileStream fileStream = new FileStream(DownloadFolderPath + filename, FileMode.Create))
                    {
                        while (true)
                        {
        ...
        public async Task<List<Ftplist>> GetFileList(string FtpFolderPath, string FtpUsername, string FtpPassword)
        {
            ...
            try
            {
                ...
                using (sourceResponse = await GetDirectoryList(FtpFolderPath, u, p, true))
                {
